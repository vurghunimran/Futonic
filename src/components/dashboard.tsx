...
type WorkItem = { id: string; title: string; client: string; workerId: string; status: "Unassigned" | "Assigned" | "In Progress" | "Completed" };
...
function DashboardHome({ clients, workers, work, onSearch, onAddWorker, onAddWork, onOpenWork }: { clients: FootballEntity[]; workers: Worker[]; work: WorkItem[]; onSearch: () => void; onAddWorker: () => void; onAddWork: () => void; onOpenWork: (item: WorkItem) => void }) {
  return <>
    ...
    <section className="panel"><PanelHeader title="Current work" note="Your manually created work and future football assignments" action={work.length ? <button className="button" onClick={onAddWork}><Plus size={15} />Add work</button> : undefined} />{work.length ? <div className="work-list">{work.map((item) => <button className="work-row" key={item.id} onClick={() => onOpenWork(item)}><div className="work-icon"><BriefcaseBusiness size={17} /></div><div><strong>{item.title}</strong><small>{item.client}</small></div><span className="badge">{item.status}</span></button>)}</div> : <EmptyState icon={<BriefcaseBusiness size={25} />} title="No work yet" text="Your dashboard starts clean. Create work only when you need it." action="Create first work" onAction={onAddWork} />}</section>
  </>;
}
