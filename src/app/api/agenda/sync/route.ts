...
export async function POST(request: Request) {
  ...
  await prisma.$transaction([
    ...
    ...parsed.data.items.map((item) => prisma.agendaItem.upsert({
      where: { id: item.id },
      create: { id: item.id, userId, kind: item.kind === "fixture" ? "FIXTURE" : "MANUAL", title: item.title, startsAt: new Date(item.startsAt), status: statuses[item.status], priority: priorities[item.priority], client: item.client, description: JSON.stringify({ competition: item.competition, home: item.home, away: item.away, selectedPlayer: item.selectedPlayer, venue: item.venue }) },
      update: { title: item.title, startsAt: new Date(item.startsAt), status: statuses[item.status], priority: priorities[item.priority], client: item.client, description: JSON.stringify({ competition: item.competition, home: item.home, away: item.away, selectedPlayer: item.selectedPlayer, venue: item.venue }) },
    })),
  ]);
  return NextResponse.json({ ok: true, synced: parsed.data.items.length });
}
