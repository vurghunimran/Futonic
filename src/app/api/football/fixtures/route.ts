...
import { selectFixturesWithinDays } from "@/lib/football/fixture-selection";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (provider === "thesportsdb") return NextResponse.json({ provider, fixtures: selectFixturesWithinDays(await getTheSportsDbFixtures(parsed.data.teamId, parsed.data.playerName)) });
    const live = Boolean(process.env.FOOTBALL_API_KEY) && provider !== "demo";
    if (live) return NextResponse.json({ provider: "api-football", fixtures: selectFixturesWithinDays(await getApiFootballFixtures(parsed.data.teamId, parsed.data.playerName)) });
    ...
    const fixtures = seed ? [0, 2, 3].map((offset, index) => ({
      ...seed,
      id: `${seed.id}-${index + 1}`,
    ...
    return NextResponse.json({ provider: "demo", fixtures: selectFixturesWithinDays(fixtures) });
  } catch (error) {
